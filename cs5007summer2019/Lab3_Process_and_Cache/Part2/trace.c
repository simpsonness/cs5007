...
// Generaetes two single values, over and over again.
void cacheTraceHit(int size, int value){
	FILE* fp = fopen("cachefriendly.txt","w");
	srand(time(NULL));
	int k = 2;
	for(int i=0; i < size; i++){
		if (k == 0) {
		...
		char* bin = intToBinaryString(value + k,BITWIDTH);
		k--;
		fprintf(fp,"%s\n",bin);
		free(bin);
	}
...
// Generaetes a 200 values, over and over again.
void cacheTraceMiss(int size, int value){
	FILE* fp = fopen("cacheunfriendly.txt","w");
...
int main(){
	...
	cacheTraceHit(ADDRESSES,0);
	cacheTraceMiss(ADDRESSES,0);
